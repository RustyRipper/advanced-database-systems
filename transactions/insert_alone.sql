...
SELECT r.parking_spot_id,
       ...
       Extract(hour FROM (TO_TIMESTAMP(:new_end_date, 'YYYY-MM-DD HH24:MI:SS') -
           ...
           -- Dodatkowa opłata za historię parkingu (średni czas rezerwacji)
       (SELECT AVG(EXTRACT(hour FROM (r2.end_date - r2.start_date))) * 0.5
        FROM Reservation r2
        WHERE r2.parking_spot_id = r.parking_spot_id
          AND r2.start_date BETWEEN SYSDATE - INTERVAL '1' MONTH AND SYSDATE)
                                                              AS amount,
       'Y'                                                    AS active,
...
WHERE r.user_id = :user_id
  ...
  AND EXISTS (SELECT 1
              FROM Reservation r3
              WHERE r3.parking_spot_id = ps.id
                AND r3.start_date BETWEEN SYSDATE - INTERVAL '30' DAY AND SYSDATE)
  AND (SELECT COUNT(*)
       FROM Reservation r4
       WHERE r4.user_id = r.user_id
         AND r4.parking_spot_id = ps.id) > 0